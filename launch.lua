...
do
	...
	EzHub.TextLabel_5.Text = " Hope you like it"
	...
	EzHub.TextLabel_6.Text = "Discord: DD()S#7777"
	...
	EzHub.ImageLabel.Image = "http://www.roblox.com/asset/?id=6240829846"
	...
	EzHub.TextLabel_8.Text = "Hello happygeneral2015, Thank you for using DD0S Hub"
...
loadToStage(0, "Setting up DD0S Hub...");
...
EzHub.TextLabel_8.Text = "Hello "..game.Players.LocalPlayer.Name..", Thank you for using DD0S Hub";
...
-----------------------------------------------
-- Display how long it took to load DD0S Hub
...
if not _G.DISABLEEXELOG then
	...
	spawn(function() ezlib.newNotif(ezlib.enum.notifType.text, "DD0S blocked EXE Logger.").play().delete(); end)
end
...
EzHub.Discord.MouseButton1Click:Connect(function()
	...
	pcall(function() setclipboard("discord.gg/fJSuja72bk") end);
	wait(2);
...
terminalPrint("Loading DD0S Hub Terminal...", "y");

local commands = {};	-- stores all commands and their functions
local tips = {
	"Ez Terminal is Ez CMD merged with DD0S Hub. This was only done because of the amount of users that wished to see Ez CMD being worked on again.",
	"Ez Terminal allows you to access many functions of DD0S Hub that you won't be able to access with a GUI button.",
	"Type cmdlist or one of it's aliases to view all the possible commands.",
	"DD0S Hub and the terminal is fully open source! Go to the Github repo to view the source."
};
...
addCommand({"cmdlist", "list", "cmds", "commands", "cmd"}, "Prints all of the command lists into the DD0S Hub terminal.", function()

	terminalPrint("DD0S Hub Terminal command list:", "b");
	for _, commandData in pairs(commands) do
...
addCommand({"quit", "close"}, "Closes DD0S Hub and unloads all of it's dependencies.", function()

	terminalPrint("Closing DD0S Hub in 3 seconds...", "r");
	spawn(function()
		wait(3);
		closeEzHub();
	end)
...
addCommand({"hide", "hidegui"}, "Hides DD0S Hub main panel. Does the same thing as pressing the toggle gui keybind.", function()
	...
	spawn(function()
		...
		terminalPrint("DD0S Hub main panel is now hiding.", "b");
	end)
...
addCommand({"guimode"}, "Outputs and accepts input only from the main DD0S Hub Terminal GUI.", function()
	if isRConsoleMode then
...
addCommand({"launch", "launchscript"}, "Launches a script from the library of DD0S Hub (Exclusives V2).", function(scriptNameArg)
...
terminalPrint("Loaded DD0S Hub Terminal successfully...", "b");
